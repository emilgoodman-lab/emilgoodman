...
export const SIZE_FACTORS=[1,.5,2,1,1.5,.75,1.5,.5,2,.75,1.5,.5,2,1];
export const MOTIF_INDICES=[0,1,3,4,6,8,9,12,15,17,18,21,23,27];
...
export class RotorField{
 constructor(width,height){this.time=0;this.motionTime=0;this.delayMix=0;this.pointer={x:0,y:0,active:false};this.weights=[1,1,1,1];this.inverted=false;this.stress=0;this.handClosed=false;this.rings=MOTIF_INDICES.map((motif,i)=>({glyph:GLYPHS[motif],motif,i,swimmer:i===4||i===10,phase:i*.371,speed:(i%2?-1:1)*(.016+(i%7)*.006),x:0,y:0,vx:0,vy:0}));this.resize(width,height);}
 ...
 step(dt){dt=clamp(dt,0,1/30);this.time+=dt;const target=this.pointer.active?zoneWeights(this.pointer.x,this.pointer.y,true):[1,1,1,1],smooth=1-Math.exp(-dt*1.1);this.weights=this.weights.map((v,i)=>v+(target[i]-v)*smooth);this.stress+=(Number(this.inverted)-this.stress)*(1-Math.exp(-dt*5));
  ...
  const gain=2*(1+this.stress);this.motionTime+=dt*.22*(1+this.stress*.25);const delayTarget=this.pointer.active?Math.max(0,1-Math.hypot(this.pointer.x*.85,(this.pointer.y+.75)*1.5)):0;this.delayMix+=(delayTarget-this.delayMix)*(1-Math.exp(-dt*.85));if(delayTarget===0&&this.delayMix<.001)this.delayMix=0;
 ...
 point(r,j){
  ...
  const glyphTurn=spin*Math.PI*.65*Math.sin(t*.45-j*.15-r.i*.31)*(1+this.stress*.4);
