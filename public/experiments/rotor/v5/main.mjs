...
function draw(){
 ...
 for(const ring of world.rings){if(ring.u===1)continue;const image=skin.sprites[ring.motif];for(let j=0;j<ring.count;j++){const p=world.point(ring,j),size=p.size*(ring.swimmer?6:3.25);if(Math.abs(p.x)>limitX+size||Math.abs(p.y)>limitY+size)continue;sctx.save();sctx.translate(p.x,p.y);sctx.rotate(p.angle);sctx.transform(Math.cos(p.localY),0,Math.sin(p.localX)*Math.sin(p.localY),Math.cos(p.localX),0,0);sctx.globalAlpha=1;const asset=ring.swimmer?swimmer.frames[frameIndex(world.time,world.delayMix,j,ring.count,ring.i,swimmer.durations)]:image;sctx.drawImage(asset,-size/2,-size/2,size,size);sctx.restore();}}
...
export function snapshot(){return {width,height,paused,hand,mirror,time:world.time,motionTime:world.motionTime,delayMix:world.delayMix,swimmerRings:world.rings.filter(r=>r.swimmer).map(r=>r.i),swimmerFrames:world.rings.filter(r=>r.swimmer).map(r=>[0,Math.floor(r.count/2)].map(j=>frameIndex(world.time,world.delayMix,j,r.count,r.i,swimmer.durations))),stress:world.stress,inverted:world.inverted,weights:[...world.weights],rings:world.rings.length,count:world.count,cursor:{...cursor},audioEnabled:audio.enabled,audioState:audio.ctx?.state};}
