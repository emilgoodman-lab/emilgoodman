...
function draw(){
 ...
 for(let j=0;j<outer.count;j++){const p=world.point(outer,j),size=p.size*6,asset=swimmer.frames[frameIndex(world.time,world.delayMix,j,outer.count,outer.i,swimmer.durations)];ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.angle);ctx.transform(Math.cos(p.localY),0,Math.sin(p.localX)*Math.sin(p.localY),Math.cos(p.localX),0,0);ctx.globalAlpha=1;ctx.drawImage(asset,-size/2,-size/2,size,size);ctx.restore();}ctx.restore();
