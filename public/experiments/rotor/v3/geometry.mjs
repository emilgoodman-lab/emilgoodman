...
export const SIZE_FACTORS=[1,.5,1,2,.5,1.25,.75,1,2,.5,1,1.5,.5,2,1,.75,1.25,.5,2,1,.75,1.5,.5,2,.75,1,.5,1];
...
export class RotorField{
 ...
 step(dt){dt=clamp(dt,0,1/30);this.time+=dt;const target=zoneWeights(this.pointer.x,this.pointer.y,this.pointer.active),smooth=1-Math.exp(-dt*3.5);this.weights=this.weights.map((v,i)=>v+(target[i]-v)*smooth);this.stress+=(Number(this.inverted)-this.stress)*(1-Math.exp(-dt*5));
  // Original field motion, with only a 1→2 gain and integrated 1→2 clock on hold.
  const gain=1+this.stress;this.motionTime+=dt*gain;
 ...
 point(r,j){
  const [depth,,spin,tilt]=this.weights,t=this.motionTime,u=r.u,a=TAU*j/r.count+r.phase,gain=1+this.stress,pin=(1-u)**.7;
  ...
  const localX=(tilt*.35+this.stress*.2)*Math.sin(t*.83-j*.13-r.i*.2),localY=(spin*.45+this.stress*.2)*Math.cos(t*.71-j*.17+r.i*.3);
  return {x:xx*perspective+r.x,y:yy*perspective+r.y,z,size:r.size*perspective,angle:tangent+glyphTurn+vibration*.2,localX,localY,alpha:clamp(.87-z/(this.unit*2.6),.16,.94)};
 }
