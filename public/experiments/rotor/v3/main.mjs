...
function draw(){
 ...
 for(const ring of world.rings){if(ring.u===1)continue;const image=skin.sprites[ring.i];for(let j=0;j<ring.count;j++){const p=world.point(ring,j),size=p.size*3.25;if(Math.abs(p.x)>limitX+size||Math.abs(p.y)>limitY+size)continue;sctx.save();sctx.translate(p.x,p.y);sctx.rotate(p.angle);sctx.transform(Math.cos(p.localY),0,Math.sin(p.localX)*Math.sin(p.localY),Math.cos(p.localX),0,0);sctx.globalAlpha=p.alpha;sctx.drawImage(image,-size/2,-size/2,size,size);sctx.restore();}}
...
async function sound(){if(audio.enabled||soundPending)return;soundPending=true;try{await audio.enable();audio.setVolume(.5);audio.setPaused(paused);}catch{document.querySelector('#camera-status').textContent='Sound unavailable. Visual controls remain active.';}finally{soundPending=false;}}
...
function release(e){if(e&&e.pointerId!==pointerId)return;pointerId=null;if(!hand){hold(false);if(e?.pointerType==='touch')cursor.active=false;}if(paused)draw();}
