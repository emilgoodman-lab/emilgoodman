...
import {loadSwimmer,frameIndex} from './swimmer.mjs';
...
const [skin,swimmer]=await Promise.all([makeCollage(),loadSwimmer()]);
...
function draw(){
 ...
 for(const ring of world.rings){if(ring.u===1)continue;for(let j=0;j<ring.count;j++){const p=world.point(ring,j),size=p.size*18;if(Math.abs(p.x)>limitX+size||Math.abs(p.y)>limitY+size)continue;sctx.save();sctx.translate(p.x,p.y);sctx.rotate(p.angle);sctx.transform(Math.cos(p.localY),0,Math.sin(p.localX)*Math.sin(p.localY),Math.cos(p.localX),0,0);sctx.globalAlpha=1;const asset=swimmer.frames[frameIndex(world.time,world.delayMix,j,ring.count,ring.i,swimmer.durations)];sctx.drawImage(asset,-size/2,-size/2,size,size);sctx.restore();}}
 ...
 for(let j=0;j<outer.count;j++){const p=world.point(outer,j),size=p.size*18,asset=swimmer.frames[frameIndex(world.time,world.delayMix,j,outer.count,outer.i,swimmer.durations)];ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.angle);ctx.transform(Math.cos(p.localY),0,Math.sin(p.localX)*Math.sin(p.localY),Math.cos(p.localX),0,0);ctx.globalAlpha=1;ctx.drawImage(asset,-size/2,-size/2,size,size);ctx.restore();}ctx.restore();
...
export function snapshot(){return {width,height,paused,hand,mirror,time:world.time,motionTime:world.motionTime,delayMix:world.delayMix,axisMix:world.axisMix,renderScale:18,swimmerRings:world.rings.map(r=>r.i),ringCounts:world.rings.map(r=>r.count),swimmerFrames:world.rings.map(r=>[0,Math.floor(r.count/2)].map(j=>frameIndex(world.time,world.delayMix,j,r.count,r.i,swimmer.durations))),selfTurns:world.rings.map(r=>[0,Math.floor(r.count/2)].map(j=>world.point(r,j).selfTurn)),stress:world.stress,inverted:world.inverted,weights:[...world.weights],rings:world.rings.length,count:world.count,cursor:{...cursor},audioEnabled:audio.enabled,audioState:audio.ctx?.state};}
