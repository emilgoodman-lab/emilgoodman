...
export class RotorField{
 ...
 resize(w,h){this.width=Math.max(1,w);this.height=Math.max(1,h);this.unit=Math.min(w,h);const base=i=>clamp(this.unit*.018*(.86+.28*((i*17%23)/23)),8,21),last=this.rings.length-1;
  ...
  const countMax=Math.max(this.unit*.25,(this.unit*.5-base(last)*1.7-10)/Math.cos(Math.PI/8));
  // Pull the paths inward enough to keep the three-times-larger silhouettes
  // visible. Element counts still derive from Rotor VI's original radii.
  const max=Math.max(this.unit*.08,(this.unit*.5-base(last)*9-10)/Math.cos(Math.PI/8));
  for(const r of this.rings){r.u=r.i/last;r.radius=this.unit*.037+r.u*(max-this.unit*.037);r.size=base(r.i)*SIZE_FACTORS[r.i];const countRadius=this.unit*.037+r.u*(countMax-this.unit*.037),perimeter=countRadius*(TAU*(1-r.u**2.4)+16*Math.sin(Math.PI/8)*r.u**2.4),denseCount=Math.max(12,Math.round(perimeter/(r.size*3.5)));r.count=Math.max(3,Math.round(denseCount/4));}
 }
