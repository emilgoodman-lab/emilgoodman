...
// Permanent per-ring asset scale: exact halves and doubles of the previous skin.
export const SIZE_FACTORS=[1,.5,2,1,.75,1.5,.5,2,.75,1];
export const MOTIF_INDICES=[0,1,3,6,9,12,17,18,23,27];
...
export class RotorField{
 constructor(width,height){this.time=0;this.motionTime=0;this.pointer={x:0,y:0,active:false};this.weights=[1,1,1,1];this.inverted=false;this.stress=0;this.handClosed=false;this.rings=MOTIF_INDICES.map((motif,i)=>({glyph:GLYPHS[motif],motif,i,phase:i*.371,speed:(i%2?-1:1)*(.016+(i%7)*.006),x:0,y:0,vx:0,vy:0}));this.resize(width,height);}
 resize(w,h){this.width=Math.max(1,w);this.height=Math.max(1,h);this.unit=Math.min(w,h);const base=i=>clamp(this.unit*.018*(.86+.28*((i*17%23)/23)),8,21),last=this.rings.length-1;
  ...
  for(const r of this.rings){r.u=r.i/last;r.radius=this.unit*.037+r.u*(max-this.unit*.037);r.size=base(r.i)*SIZE_FACTORS[r.i];const perimeter=r.radius*(TAU*(1-r.u**2.4)+16*Math.sin(Math.PI/8)*r.u**2.4);r.count=Math.max(12,Math.round(perimeter/(r.size*1.38)));}
 }
 ...
 step(dt){dt=clamp(dt,0,1/30);this.time+=dt;const target=this.pointer.active?zoneWeights(this.pointer.x,this.pointer.y,true):[1,1,1,1],smooth=1-Math.exp(-dt*3.5);this.weights=this.weights.map((v,i)=>v+(target[i]-v)*smooth);this.stress+=(Number(this.inverted)-this.stress)*(1-Math.exp(-dt*5));
  // All fields idle at full strength, with a 2× base motion gain and clock.
  const gain=2*(1+this.stress);this.motionTime+=dt*gain;
  for(const r of this.rings){r.phase+=r.speed*dt*gain;if(r.u===1){r.x=r.y=r.vx=r.vy=0;continue;}const lag=.15+r.u*.9,k=20/(1+lag*5),drag=2*Math.sqrt(k)*.84,wave=this.motionTime*.58-r.i*.18,pin=(1-r.u)**.7;
 ...
 point(r,j){
  ...
  const glyphTurn=spin*Math.PI*Math.sin(t*1.15-j*.15-r.i*.31)*gain;
  const localX=(tilt*.35+this.stress*.2)*Math.sin(t*.83-j*.13-r.i*.2)*gain,localY=(spin*.45+this.stress*.2)*Math.cos(t*.71-j*.17+r.i*.3)*gain;
  return {x:xx*perspective+r.x,y:yy*perspective+r.y,z,size:r.size*perspective,angle:tangent+glyphTurn+vibration*.2,localX,localY,alpha:1};
 }
