...
export class RotorField{
 constructor(width,height){this.time=0;this.pointer={x:0,y:0,active:false};this.weights=[0,0,0,0];this.inverted=false;this.stress=0;this.handClosed=false;this.rings=GLYPHS.map((glyph,i)=>({glyph,i,phase:i*.371,speed:(i%2?-1:1)*(.016+(i%7)*.006),x:0,y:0,vx:0,vy:0}));this.resize(width,height);}
 ...
 hand(tracked,x=0,y=0,fist=false){if(!tracked){this.handClosed=false;this.pointer.active=false;return;}this.move(x,y);if(fist&&!this.handClosed)this.toggle();this.handClosed=fist;}
 step(dt){dt=clamp(dt,0,1/30);this.time+=dt;const target=zoneWeights(this.pointer.x,this.pointer.y,this.pointer.active),smooth=1-Math.exp(-dt*3.5);this.weights=this.weights.map((v,i)=>v+(target[i]-v)*smooth);this.stress+=(Number(this.inverted)-this.stress)*(1-Math.exp(-dt*5));
  for(const r of this.rings){const lag=.15+r.u*.9,k=20/(1+lag*5),drag=2*Math.sqrt(k)*.84;const wave=this.time*.58-r.i*.18;const x=this.weights[1]*this.unit*(this.pointer.x*.11+Math.sin(wave)*.052),y=this.weights[1]*this.unit*(this.pointer.y*.11+Math.cos(wave*.83)*.052);r.vx+=((x-r.x)*k-r.vx*drag)*dt;r.vy+=((y-r.y)*k-r.vy*drag)*dt;r.x+=r.vx*dt;r.y+=r.vy*dt;r.phase+=r.speed*dt;}
 }
 point(r,j){
  const [depth,,spin,tilt]=this.weights,t=this.time,u=r.u,a=TAU*j/r.count+r.phase;
  const vibration=this.stress*(Math.sin(t*31+r.i*1.7+j*.22)*.65+Math.sin(t*19-r.i+j*.49)*.35);
  const rr=r.radius+vibration*this.unit*.009;
  let x=Math.cos(a)*rr,y=Math.sin(a)*rr;
  const ax=tilt*(.12+u*.91)*Math.sin(t*.27+u*1.8),ay=tilt*(.15+u*.98)*Math.cos(t*.21+u*1.3);
  ...
  const glyphTurn=spin*Math.PI*Math.sin(t*1.15-j*.15-r.i*.31);
  return {x:baseX,y:baseY,z,size:r.size*perspective,angle:tangent+glyphTurn+vibration*.1,alpha:clamp(.87-z/(this.unit*2.6),.16,.94)};
 }
