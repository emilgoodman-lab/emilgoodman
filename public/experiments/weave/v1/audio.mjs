...
export function buildWeaveAudio(ctx){
 ...
 const lfo=ctx.createOscillator(),lfoGain=ctx.createGain();lfo.type='sine';lfo.frequency.value=.24;lfoGain.gain.value=.012;lfo.connect(lfoGain).connect(calmBus.gain);lfo.start();
 ...
 let lastDrive=-1;
 const update=(state,at=ctx.currentTime)=>{const p=state.pointer||{x:state.x??.5,y:state.y??.5},nx=state.x??(state.width?p.x/state.width:.5),ny=state.y??(state.height?p.y/state.height:.5),dwell=clamp(state.dwell??0,0,1),knot=clamp(state.knot??0,0,1),energy=clamp(state.energy??0,0,1),wave=clamp(state.waveEnergy??energy,0,1),base=92*Math.pow(2,clamp(nx,0,1)*1.35)+clamp(ny,0,1)*22,pulse=.18+dwell*.48;
  calmBus.gain.setTargetAtTime((.006+dwell*.065+wave*.028)*(1-knot*.8),at,.06);filter.frequency.setTargetAtTime(650+ny*1250+dwell*1000,at,.09);pan.pan.setTargetAtTime(clamp(nx*2-1,-.85,.85),at,.08);airFilter.frequency.setTargetAtTime(330+nx*760,at,.12);airGain.gain.setTargetAtTime(.008+dwell*.026+energy*.02,at,.08);lfo.frequency.setTargetAtTime(.16+dwell*.82,at,.12);lfoGain.gain.setTargetAtTime(pulse*(.012+dwell*.038),at,.1);
  voices.forEach((v,i)=>{v.osc.frequency.setTargetAtTime(base*v.ratio*(1+Math.sin((state.time||0)*(.19+i*.06))*0.006),at,.06);v.gain.gain.setTargetAtTime((i?0.027:0.05)*(1+dwell*.7),at,.08);});
 ...
 const dispose=()=>{[...voices.map(v=>v.osc),...chaos.map(v=>v.osc),lfo,trem,air].forEach(n=>{try{n.stop();}catch{}});master.disconnect();};
