...
function telemetry(force=false){if(!force&&world.time-lastTelemetry<.2)return;lastTelemetry=world.time;const c=world.chains[7];readout.textContent=[`chains   = ${world.chains.length}`,`links    = ${world.chains.reduce((n,c)=>n+c.nodes.length,0)}`,`step     = 120 Hz`,`time     = ${world.time.toFixed(2)} s`,`input    = ${hand?'HAND':'POINTER'}`,`chain[7] = ${c.energy.toFixed(1)} px/s`,`sound    = ${audio.fist?'DISTORT':'CHIMES'}`,`tone[7]  = ${frequencies[7].toFixed(1)} Hz`].join('\n');}
...
window.addEventListener('lab-hand',e=>{hand=Boolean(e.detail.tracked);audio.setFist(hand&&Boolean(e.detail.fist));if(!hand){world.release();lastHand=0;return;}const now=performance.now();world.move((e.detail.x+1)*.5*width,(e.detail.y+1)*.5*height,lastHand?(now-lastHand)/1000:.05,e.detail.fist);lastHand=now;});
