...
export class ChainWorld{
 constructor(width,height){this.pointer={active:false,x:0,y:0,fromX:0,fromY:0,vx:0,vy:0,down:false};this.grab=null;this.fist=false;this.time=0;this.resize(width,height);}
 resize(width,height){
  ...
  if(!this.chains){this.chains=Array.from({length:16},(_,id)=>({id,nodes:[],count:62+(id%2),length:0,energy:0,contact:{active:false,strength:0,height:.5}}));}
  for(const chain of this.chains){
   ...
   for(let j=0;j<chain.count;j++){
    let p=chain.nodes[j];
    if(!p){const angle=.018*Math.sin(chain.id*1.7);p={x:x+Math.sin(angle)*j*chain.rest,y:y+Math.cos(angle)*j*chain.rest,px:0,py:0,inv:j===0?0:1,id:chain.id*100+j,vibration:0,size:.75+.5*randomSize(chain.id*100+j)};p.px=p.x;p.py=p.y;chain.nodes.push(p);}
 ...
 setFist(value){this.fist=Boolean(value);if(this.fist){this.grab=null;this.pointer.down=false;}}
 release(){this.pointer.active=false;this.pointer.down=false;this.pointer.vx=this.pointer.vy=0;this.grab=null;for(const c of this.chains)c.contact.active=false;}
 solveDistances(){
 ...
 step(dt=STEP){
  ...
  for(const c of this.chains){
   let weight=0,heightSum=0,strongest=0;
   for(let j=1;j<c.nodes.length;j++){
   const p=c.nodes[j];let ax=0,ay=gravity;p.vibration*=Math.exp(-9*dt);
   if(cursor.active&&!this.grab){
    ...
    if(distance<reach){const force=(1-distance/reach)**2;ax=cursor.vx*14*force;ay+=cursor.vy*14*force;
     weight+=force;heightSum+=clamp(p.y/this.height,0,1)*force;strongest=Math.max(strongest,force);
     if(this.fist)p.vibration=Math.max(p.vibration,force);
    }
   }
   const x=p.x,y=p.y;p.x+=(p.x-p.px)*damp+ax*dt*dt;p.y+=(p.y-p.py)*damp+ay*dt*dt;p.px=x;p.py=y;
  }
   c.contact={active:weight>0&&strongest>.035,strength:strongest,height:weight?heightSum/weight:c.contact.height};
  }
  for(let i=0;i<24;i++){
   this.solveDistances();
  ...
  cursor.vx*=Math.exp(-12*dt);cursor.vy*=Math.exp(-12*dt);cursor.fromX=cursor.x;cursor.fromY=cursor.y;this.time+=dt;
 }
