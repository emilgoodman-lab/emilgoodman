...
(async () => {
  ...
  function render(){
    ...
    const radius=Math.min(width*.34,height*.36,410);
    const glyphSize=width<600?10.5:16;
    const transforms=layers.map((layer,l)=>{
      const rot=time*layer.speed+layer.x*layer.turn;
      const tilt=layer.y*(.42+l*.14)+Math.sin(time*(.14+l*.025)+layer.phase)*.18;
      layer.rotation=rot;
      return {cr:Math.cos(rot),sr:Math.sin(rot),ct:Math.cos(tilt),st:Math.sin(tilt),
        breath:1+.045*Math.sin(time*(.7+l*.19)+layer.phase),
        dx:layer.x*radius*(.12+l*.095)+Math.sin(time*.31+layer.phase)*radius*.018+(l-1)*spread*radius*.3,
        dy:layer.y*radius*(.09+l*.075)+Math.cos(time*.24+layer.phase)*radius*.018};
    });
    for(const p of points){
      ...
      const r=radius*layer.scale*(m.breath+wave)*(1+spread*(.1+p.l*.22))*p.thickness;
  ...
  function still(){if(paused){for(const layer of layers){layer.x=targetX;layer.y=targetY;}spread=holding?1:0;render();telemetry(performance.now(),true);}}
